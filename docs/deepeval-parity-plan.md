...
- Optimizer prompt configuration/report value records, SIMBA trace bucket records, and algorithm configuration surfaces for SIMBA, COPRO, and GEPA with DeepEval-style defaults, seeded random state handling, runner status enum, tie-breaker policy enum, GEPA bounds validation, tie handling, Pareto frontier, frequency weighting, and Pareto sampling helpers
