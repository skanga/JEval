...
- Single-turn synthesizer for contexts, docs, scratch styling, existing goldens, text-to-SQL schema contexts, and Java `CompletableFuture` async generation wrappers, with inferred prompt styling for existing-golden generation including contextual and explicit-empty-context goldens, cross-file source-label prompt rendering, cross-file reasoning objective prompts, target-file prompt instructions, DeepEval-style `used_source_files` JSON examples, path-preserving document source labels, cross-file document context merging, PDF/DOCX/text/Markdown document loading, extension validation, explicit text document encoding, UTF-8 BOM stripping for text loaders, markdown-family intra-chunk whitespace/table preservation, DeepEval-style context length controls, CLI generation including cross-file docs flags and `--include-expected/--no-include-expected`, and OpenAI/OpenRouter/Ollama LangChain4j provider wiring
...
- Multi-turn synthesizer for contexts, docs, scratch styling, existing conversational goldens, and Java `CompletableFuture` async generation wrappers, producing `ConversationalGolden` values with scenario, turns, expected outcome, context, metadata, inferred conversational styling for existing-golden generation including contextual and explicit-empty-context goldens, cross-file source-label prompt rendering, cross-file reasoning objective prompts, target-file prompt instructions, DeepEval-style `used_source_files` JSON examples, path-preserving document source labels, cross-file document context merging, PDF/DOCX/text/Markdown document loading, extension validation, explicit text document encoding, UTF-8 BOM stripping for text loaders, markdown-family intra-chunk whitespace/table preservation, per-document minimum-context failure continuation when later documents remain usable, CLI generation including cross-file docs flags, and DeepEval-style embedded/trailing-comma schema JSON parsing
...
- Simulator prompt templates for first user turn, next user turn, expected-outcome completion checks, and simulation-graph edge classification
- Simulator graph runner with per-conversation state, max-visit caps, terminal emissions, no-edge short-circuiting, and model-routed edge advancement
- Simulator controller helpers for proceed/end decisions, context construction, custom controller normalization, and expected-outcome completion checks
- Conversation simulator utility surface for first/next simulated user input generation, DeepEval-style embedded/trailing-comma simulator JSON parsing, language defaults, assistant callback invocation, custom simulation graph/controller wiring, reusable default simulation nodes with custom prompt templates and simulator-aware action contexts, retained simulated conversations, completion callbacks, and synchronous multi-golden simulation into `ConversationalTestCase` values with DeepEval-default max-turn and expected-outcome stopping
- Optimizer async/display config records with PromptOptimizer storage, prompt configuration/report value records, model callback invocation, synchronous scorer shell with supplied-output evaluation and minibatch feedback diagnosis blocks, callback-driven rewriter/template shell, PromptOptimizer delegation wrapper, GEPA root-configuration execution/reporting, COPRO root-configuration/bootstrap/history execution/reporting with depth iteration logs, SIMBA root-configuration execution/reporting with introspective iteration logs, MIPROv2 root-configuration and candidate/demo-pair execution/reporting with trial iteration logs, COPRO proposer templates and callback-driven candidate generation, SIMBA introspection rewrite templates, callback-driven proposer rewriting, demo injection, proposer-backed candidate execution, and acceptance reporting, MIPROv2 dataset-summary and instruction-proposal templates with callback-driven candidate generation, MIPROv2 demonstration records, set construction, DeepEval-style prompt rendering, and scorer-backed bootstrapped/labeled demonstration filtering, GEPA golden-count validation, GEPA prompt-equivalence, minibatch, child configuration, Pareto acceptance, archive-pruning, elapsed iteration log state, and aggregate best-selection helpers, prompt parse/recreate utilities, report snapshot/golden split/validation utilities including metric-list validation, SIMBA trace bucket records, and algorithm configuration surfaces for SIMBA, COPRO, GEPA, and MIPROv2 with DeepEval-style defaults, seeded random state handling, runner status enum, tie-breaker policy enum, GEPA bounds validation, tie handling, Pareto frontier, frequency weighting, and Pareto sampling helpers
- BigBenchHard benchmark evaluation over task-grouped supplied `Golden` rows with exact-match task prediction rows, task scores, batch support, and `overallScore`
...
- EquityMedQA benchmark evaluation over task-grouped supplied `Golden` rows with generated-answer bias-metric scoring, task scores, and `overallScore`
...
- SQuAD benchmark evaluation over task-grouped supplied `Golden` rows with evaluator-model answer scoring, task scores, and `overallScore`
