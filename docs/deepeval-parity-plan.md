...
- Single-turn synthesizer for contexts, docs, scratch styling, existing goldens, text-to-SQL schema contexts, and Java `CompletableFuture` async generation wrappers, with cross-file source-label prompt rendering, cross-file reasoning objective prompts, target-file prompt instructions, DeepEval-style `used_source_files` JSON examples, path-preserving document source labels, cross-file document context merging, PDF/DOCX/text/Markdown document loading, extension validation, UTF-8 BOM stripping for text loaders, markdown-family intra-chunk whitespace/table preservation, DeepEval-style context length controls, CLI generation including cross-file docs flags and `--include-expected/--no-include-expected`, and OpenAI/OpenRouter/Ollama LangChain4j provider wiring
...
- Multi-turn synthesizer for contexts, docs, scratch styling, existing conversational goldens, and Java `CompletableFuture` async generation wrappers, producing `ConversationalGolden` values with scenario, turns, expected outcome, context, metadata, cross-file source-label prompt rendering, cross-file reasoning objective prompts, target-file prompt instructions, DeepEval-style `used_source_files` JSON examples, path-preserving document source labels, cross-file document context merging, PDF/DOCX/text/Markdown document loading, extension validation, UTF-8 BOM stripping for text loaders, markdown-family intra-chunk whitespace/table preservation, per-document minimum-context failure continuation when later documents remain usable, CLI generation including cross-file docs flags, and DeepEval-style embedded/trailing-comma schema JSON parsing
