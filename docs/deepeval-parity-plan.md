...
- Single-turn synthesizer for contexts, docs, scratch styling, existing goldens, and Java `CompletableFuture` async generation wrappers, with cross-file source-label prompt rendering and cross-file document context merging, CLI generation, and OpenAI/OpenRouter/Ollama LangChain4j provider wiring
