...
- Conversation simulator utility surface for first/next simulated user input generation, structured simulator-model JSON parsing, language defaults, assistant callback invocation, and synchronous multi-golden simulation into `ConversationalTestCase` values with max-turn and expected-outcome stopping
